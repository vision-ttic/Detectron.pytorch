...
The overall coding style here is declarative.
The boxes, segms, etc are first declared as empty arrays with right shapes in
_prep_roidb_entry, and then filled in either through
    _add_gt_from_cache or _add_gt_annotations
    _add_proposals_from_file
This declarative style is also apparent later in box regression and cls target
calculation. Good style?
"""
    ...
    def get_roidb(
        ...
        if gt:
            ...
            cache_filepath = os.path.join(
                self.cache_path, self.name + '_gt_roidb.pkl')
    ...
    def _add_gt_annotations(self, entry):
        ...
        for ix, obj in enumerate(valid_objs):
            ...
            if obj['iscrowd']:
                ...
                # HC: this means that crowd proposals will not be fed as ROI
                # However, crowd boxes can still serve as a gt. Subtle
                gt_overlaps[ix, :] = -1.0
            else:
...
def add_proposals(roidb, rois, scales, crowd_thresh):
    ...
    HC:
        This is called by modeling/generate_proposal_labels.py to inject
        propoals generated by RPN into roidb on the fly. Sounds gross
        Note that rois are of shape [R, 5] where [:, 0] denotes the batch ind
        of the roidb to which the roi belongs. This restores batches.
    """
    # box_list and roidb should come in pairs, hence restoring batches
    box_list = []
