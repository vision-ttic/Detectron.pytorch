...
class Generalized_RCNN(nn.Module):
    def __init__(self):
        """
        HC:
            RCNN Construction sequence. ? indicates optional
            conv_body
            RPN?
            FPN?
            bbox?
        ...
        conv_body_func = get_func(cfg.MODEL.CONV_BODY)
        # break it up for easier debugger walkthrough
        ...
        if cfg.FPN.FPN_ON:
            ...
            # Retain only the spatial scales that will be used for RoI heads.
            # `Conv_Body.spatial_scale` may include extra scales that are used
            # for RPN proposals, but not for RoI heads.
            self.Conv_Body.spatial_scale = \
                self.Conv_Body.spatial_scale[-self.num_roi_levels:]

        if not cfg.MODEL.RPN_ONLY:
            # BBOX Branch
            # HC: box_head uses Res5 to crop and transform features to ROI fea
            self.Box_Head = get_func(cfg.FAST_RCNN.ROI_BOX_HEAD)(
                self.Conv_Body.dim_out,
                self.roi_feature_transform,
                self.Conv_Body.spatial_scale
            )
            # HC: box_outs takes ROI features to get bbox and class
    ...
    def _init_modules(self):
        if cfg.MODEL.LOAD_IMAGENET_PRETRAINED_WEIGHTS:
            ...
            if cfg.MODEL.KEYPOINTS_ON and \
                    getattr(self.Keypoint_Head, 'SHARE_RES5', False):
                assert compare_state_dict(
    ...
    def _forward(self, data, im_info, roidb=None, **rpn_kwargs):
        """
        HC:
            Initially data is batched images.
            RPN outputs ROI [N_BOXES, 5] that records batch index in [:, 0]
            ROI croppping operates on ROI, with batch agnostic output as well
        """
        im_data = data
        if self.training:
        ...
        # [NUM_IMGS, C_at_Res4(1024), H / stride, W / stride]
        blob_conv = self.Conv_Body(im_data)
        # ------------- Henceforth, batch is destroyed. --------------
        ...
        if not cfg.MODEL.RPN_ONLY:
            ...
            # HC: the legacy path was left by Roy for RPN_ONLY train. Unnecessary
            pass

        if self.training:
            return_dict['losses'] = {}
            return_dict['metrics'] = {}
            # rpn and bbox loss. WARNING does not even support Fast RCNN training
            if cfg.RPN.RPN_ON:
                rpn_kwargs.update(dict(
                    (k, rpn_ret[k])
                    for k in rpn_ret.keys()
                    if (k.startswith('rpn_cls_logits') or
                        k.startswith('rpn_bbox_pred'))
                ))
                loss_rpn_cls, loss_rpn_bbox = \
                    rpn_heads.generic_rpn_losses(**rpn_kwargs)
                if cfg.FPN.FPN_ON:
            ...
            if not cfg.MODEL.RPN_ONLY:
                ...
                loss_cls, loss_bbox, accuracy_cls = \
                    fast_rcnn_heads.fast_rcnn_losses(
    ...
    def roi_feature_transform(
        self, blobs_in, rpn_ret, blob_rois='rois', method='RoIPoolF',
